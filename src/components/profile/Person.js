...
export default class Person extends React.Component {
    ...
    componentWillMount() {
        AsyncStorage.getItem('profile', (err, profile) => {
            ...
            this.setState({ mobile: response.mobile, IDProof: response.idtype ? response.idtype : 'Aadhar Card', location: response.location ? response.location : 'Bangalore', name: response.name ? response.name : 'Your name', age: response.age ? response.age : 0, gender: response.gender ? response.gender : 'Male', profession: response.profession ? response.profession : 'Software Developer', prefferedLanguage: response.prefferedLanguage ? response.prefferedLanguage : 'English', ImageSource: response.profilePhotoURL ? { uri: response.profilePhotoURL } : require('../../assets/images/profile.png'), IDProofImg: response.idFileURL ? { uri: response.idFileURL } : {} })
            ...
            <KeyboardAvoidingView behavior="padding" style={{ flex: 1 }}>
                <ScrollView style={{ backgroundColor: '#fff' }}>
                    ...
                    <View style={{ padding: 10 }}>
                        ...
                        <Text style={{height: 23,borderTopColor: '#ffffff',borderBottomColor: '#B3B3B3',
                                    borderLeftColor: '#ffffff',borderRightColor: '#ffffff',borderWidth: 1,
                                    marginTop:5,margin: 0,color:'#B3B3B3'}} >{this.state.location}
                        </Text>
                    ...
                    <View style={styles.buttonContainer}>
                        ...
                        <View style={styles.buttonStyle}>
                            <ButtonC title="LOG OUT" onclick={this.handleLog} />
                        </View>
                    </View>
