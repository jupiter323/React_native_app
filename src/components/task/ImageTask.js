...
export default class ImageTask extends React.Component {
	...
	// 		console.log('uploaded image url', this.state.ImageSourceArray[this.state.noOfFiles - this.state.index])
	// 		this.state.api.mediaHandle(this.state.ImageSourceArray[this.state.noOfFiles - this.state.index], 'images/', '.jpg', "image/jpeg").then(res => {
	// 			var tempJSON = JSON.parse(this.state.task.definitionJSON);
	// 			tempJSON.taskDefinition = res;
	// 			tempJSON.noOfFiles = --this.state.index;
	...
	handleSubmit = () => {
		if (this.state.imageuploaded) {
			...
			var tempJSON = JSON.parse(this.state.task.definitionJSON);
			tempJSON.taskDefinition = JSON.stringify({ "ImageSourceArray": this.state.ImageSourceArray });
			Object.assign(tempJSON, { "userComment": this.state.expText })
			this.state.task.definitionJSON = JSON.stringify(tempJSON);
			console.log(this.state.task.definitionJSON)
